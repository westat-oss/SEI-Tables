...
# the number of U.S. and non-U.S. collaborations (using branch as the identifier) for each unique country. 
...
#_______________________________________________________________________________ 
 ...
 generate_FigureINV_A <- function(
   ...
   joined_data <- commits %>%
     ...
     select(branch, commit_year, country)
   ...
   collaborations_with_us <- joined_data %>%
     semi_join(repos_with_us, by = "branch") %>%  # keep only branches that have U.S. presence
     filter(country != us_country) %>%
     group_by(country) %>%
     summarise(collaborations_with_us = n_distinct(branch), .groups = "drop")
   
   ## 7) Count collaborations among NON-U.S. countries ----
   collaborations_non_us <- joined_data %>%
     filter(country != us_country) %>%
     group_by(country) %>%
     summarise(collaborations_with_non_us = n_distinct(branch), .groups = "drop")
   
   ## 8) Merge both counts, fill NAs so that "Missing Country" is retained, and take the top 10 ----
   combined_table <- collaborations_with_us %>% 
     full_join(collaborations_non_us, by = "country") %>%
     mutate(
   ...
   ## 9) Pull the result into R memory and return it ----
   ...
   ## 10) Write the result to the excel workbook ----
 ...
 # Print the result to see the output for 2009
 print(result_2023)
